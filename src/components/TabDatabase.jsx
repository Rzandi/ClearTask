...
import MergePreviewModal from './MergePreviewModal.jsx';
...
export default function TabDatabase() {
  ...
  // ── Read data from hooks (auto-updates via Dexie useLiveQuery) ──
  ...
  function handleFileChange(e) {
    ...
    reader.onload = async (event) => {
      ...
      const merge = await calculateMerge(result.data);
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div>
        ...
        <p className="text-sm text-text-muted">Kelola data transaksi dan inventaris barang Anda.</p>
      </div>

      {/* Sub-tab Navigation */}
      <div
        className="flex gap-1 p-1 bg-bg-elevated rounded-xl border border-border-subtle"
        role="tablist"
        aria-label="Sub-tab database"
      >
        <button
          role="tab"
      ...
      {subTab === 'inventaris' ? (
        ...
        <div role="tabpanel" id="panel-data" className="space-y-6 mt-2">
          ...
          <div className="overflow-x-auto rounded-xl border border-border-default">
            <table className="w-full text-sm" data-testid="tabel-semua-transaksi">
              <thead>
                ...
                {filteredTransactions.length === 0 ? (
                  ...
                  filteredTransactions.map((tx, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-text-secondary">{tx.metode || '—'}</td>
...
function StatCard({ label, value }) {
  return (
    <div className="glass-card p-5 animate-slide-up">
      ...
      <p className="text-2xl lg:text-3xl font-extrabold text-primary tracking-tight">{value}</p>
    </div>
  );
}
