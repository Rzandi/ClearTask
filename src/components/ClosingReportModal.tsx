      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-border-default shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="closing-report-title" className="text-lg font-semibold text-text-primary">
                Closing Report
              </h2>
              {/* 11.2 Nama sesi */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-bg-elevated rounded-xl p-4 border border-border-subtle">
                ...
                <p className="text-sm font-medium text-text-primary">
                  {session?.waktuMulai
                    ? `${formatDate(session.waktuMulai)}, ${formatTime(session.waktuMulai)}`
                    : '-'}
                </p>
              </div>
              <div className="bg-bg-elevated rounded-xl p-4 border border-border-subtle">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border-subtle">
                        <th className="text-left px-4 py-3 text-xs font-semibold text-text-muted">
                          Kategori
                        </th>
                        <th className="text-right px-4 py-3 text-xs font-semibold text-text-muted">
                          Transaksi
                        </th>
                        <th className="text-right px-4 py-3 text-xs font-semibold text-text-muted">
                          Total
                  ...
                  <div className="bg-bg-elevated rounded-xl border border-border-subtle overflow-hidden max-h-[220px] overflow-y-auto">
                    <table className="w-full text-[11px] sm:text-sm">
                      <thead className="sticky top-0 bg-bg-surface border-b border-border-subtle z-10">
                        <tr>
                          <th className="text-left px-2 sm:px-4 py-2 text-xs font-semibold text-text-muted">
                            Nama Barang
                          </th>
                          <th className="text-center px-2 sm:px-4 py-2 text-xs font-semibold text-text-muted">
                            Terjual
                          </th>
                          <th className="text-right px-2 sm:px-4 py-2 text-xs font-semibold text-text-muted">
                            Total Penjualan
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {soldItemsBreakdown.map((item, idx) => (
                          <tr
                            key={idx}
                            className="border-b border-border-subtle last:border-0 hover:bg-white/[0.01]"
                          >
                            <td className="px-2 sm:px-4 py-2 sm:py-2.5 text-text-primary font-medium truncate max-w-[110px] sm:max-w-[180px]">
                              {item.namaBarang}
                            </td>
                            <td className="px-2 sm:px-4 py-2 sm:py-2.5 text-center text-text-secondary font-bold">
                              {item.qty}
                            </td>
                            <td className="px-2 sm:px-4 py-2 sm:py-2.5 text-right text-text-primary">
                  ...
                  <div className="bg-bg-elevated rounded-xl p-4 border border-border-subtle flex items-center justify-center">
                    <p className="text-sm text-text-secondary font-medium">
                      ...
                      <strong className="text-text-primary">
                        {stats.breakdownMetode[0]?.metode}
                      </strong>
                    </p>
                  </div>
                ) : (
