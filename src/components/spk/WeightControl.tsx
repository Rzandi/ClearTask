...
export const WeightControl: React.FC<WeightControlProps> = ({
  ...
  useEffect(() => {
    const t = Math.round(
      (weights.c1_weight + weights.c2_weight + weights.c3_weight + weights.c4_weight) * 100
    );
    setTotal(t);
  }, [weights]);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b border-border-subtle pb-4">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">
            Sesuaikan persentase kepentingan setiap kriteria (Total harus 100%).
          </p>
        </div>
        <div
          className={`mt-4 sm:mt-0 px-4 py-1.5 rounded-full text-sm font-bold border ${isTotalValid ? 'bg-primary/10 text-primary border-primary/20 shadow-glow' : 'bg-error/10 text-error border-error/20'}`}
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-bg-input border border-border-default rounded-xl p-4 shadow-inner">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-[10px] px-2 py-1 bg-primary/10 text-primary border border-primary/20 rounded font-bold">
              Benefit
            </span>
          </div>
        ...
        <div className="bg-bg-input border border-border-default rounded-xl p-4 shadow-inner">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-[10px] px-2 py-1 bg-warning/10 text-warning border border-warning/20 rounded font-bold">
              Cost
            </span>
          </div>
          <div className="flex items-center gap-3">
