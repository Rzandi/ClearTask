  ...
  if (!data || data.length === 0) {
    return (
      <div className="glass-card p-6 flex flex-col items-center justify-center h-64 mb-6">
        ...
        <p className="text-text-secondary font-medium">Belum ada data transaksi untuk periode ini.</p>
      </div>
    );
  }
  ...
  return (
    <div className="glass-card p-4 sm:p-6 mb-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b border-border-subtle pb-4">
        <h2 className="text-lg font-semibold text-text-primary">Visualisasi Top {topN} Prioritas</h2>
      ...
      <div className="space-y-4">
        {chartData.map((item, index) => {
          ...
          if (item.urgency_level === 'urgent') barColor = 'bg-error shadow-[0_0_15px_rgba(255,51,102,0.5)]';
          else if (item.urgency_level === 'perhatian') barColor = 'bg-warning shadow-[0_0_15px_rgba(240,180,41,0.5)]';

          return (
            <div key={item.product_name} className="flex items-center">
              <div className="w-1/3 pr-4 flex items-center justify-end">
                <span className="text-sm font-medium text-text-primary truncate text-right" title={item.product_name}>
                  {item.product_name}
                </span>
