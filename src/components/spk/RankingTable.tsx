          ...
          <>
            ...
            <table className="min-w-full divide-y divide-border-default text-sm">
              ...
              <tr>
                <th
                  scope="col"
                  className="px-6 py-3 text-left font-semibold text-text-secondary uppercase tracking-wider"
                >
                  Rank
                </th>
                <th
            ...
            <tbody className="divide-y divide-border-default bg-bg-surface">
              {results.length === 0 ? (
                ...
                visibleResults.map((r, idx) => (
                  <tr
                    key={idx}
                    className={`transition-colors hover:bg-bg-elevated ${r.urgency_level === 'urgent' ? 'bg-error/5' : ''}`}
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${
                          r.rank === 1
                            ? 'bg-warning text-bg-base shadow-[0_0_10px_rgba(240,180,41,0.5)]'
                            : r.rank === 2
                              ? 'bg-text-secondary text-bg-base shadow-glow'
                              : r.rank === 3
                                ? 'bg-orange-500 text-bg-base shadow-[0_0_10px_rgba(249,115,22,0.5)]'
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2.5 py-1 inline-flex text-xs font-bold rounded-md border 
                        ${
                          r.urgency_level === 'urgent'
                            ? 'bg-error/10 text-error border-error/20'
                            : r.urgency_level === 'perhatian'
          ...
          <div className="p-4">
            <table className="min-w-full divide-y divide-border-default text-sm border border-border-default rounded-xl overflow-hidden">
              <thead className="bg-bg-input">
                <tr>
                  <th
                    scope="col"
                    className="px-4 py-2 border-r border-border-default text-center text-text-secondary font-semibold"
                    rowSpan={2}
                  >
                    Nama Produk
                  </th>
                  ...
                  <th
                    scope="col"
                    className="px-4 py-2 text-center text-text-secondary font-semibold"
                    rowSpan={2}
                  >
                    Vi (Akhir)
                  </th>
                ...
                <tr className="bg-bg-surface">
                  <th
                    scope="col"
                    className="px-2 py-1 border-r border-border-subtle text-center text-[10px] text-text-muted"
                  >
                    Raw
                  </th>
                  <th
                    scope="col"
                    className="px-2 py-1 border-r border-border-default text-center text-[10px] text-primary font-bold"
                  >
                    Norm
                  </th>
                  <th
                ...
                {results.length === 0 ? (
                  ...
                  visibleResults.map((r, idx) => (
                    <tr key={idx} className="hover:bg-bg-elevated transition-colors">
                      <td className="px-4 py-2 font-medium border-r border-border-default text-text-primary">
                        {r.product_name}
                      </td>
                      <td className="px-4 py-2 text-right border-r border-border-subtle text-text-secondary">
                        {r.raw_values.c1_volume}
                      </td>
                      <td className="px-4 py-2 text-right font-mono border-r border-border-default text-text-primary">
                        {r.normalized_values.c1.toFixed(4)}
                      </td>
                      <td className="px-4 py-2 text-right border-r border-border-subtle text-text-secondary">
                        {r.raw_values.c2_stock}
                      </td>
                      <td className="px-4 py-2 text-right font-mono border-r border-border-default text-text-primary">
                        {r.normalized_values.c2.toFixed(4)}
                      </td>
                      <td className="px-4 py-2 text-right border-r border-border-subtle text-text-secondary">
                        {r.raw_values.c3_margin_pct.toFixed(1)}%
                      </td>
                      <td className="px-4 py-2 text-right font-mono border-r border-border-default text-text-primary">
                        {r.normalized_values.c3.toFixed(4)}
                      </td>
                      <td className="px-4 py-2 text-right border-r border-border-subtle text-text-secondary">
                        {r.raw_values.c4_frequency}
                      </td>
                      <td className="px-4 py-2 text-right font-mono border-r border-border-default text-text-primary">
                        {r.normalized_values.c4.toFixed(4)}
                      </td>
                      <td className="px-4 py-2 text-right font-bold bg-primary/10 text-primary border-l border-primary/20">
                        {r.score.toFixed(4)}
                      </td>
                    </tr>
                  ))
            ...
            <div className="mt-4 p-4 bg-primary/5 border border-primary/20 rounded-xl text-sm text-text-secondary shadow-inner">
              ...
              <ul className="list-disc pl-5 space-y-1">
                <li>
                  <strong>Benefit (C1, C3, C4):</strong>{' '}
                  <code className="bg-bg-input border border-border-subtle px-1.5 py-0.5 rounded text-xs font-mono text-text-primary">
                    r_ij = x_ij / max(x_j)
                  </code>
                </li>
                <li>
                  <strong>Cost (C2):</strong>{' '}
                  <code className="bg-bg-input border border-border-subtle px-1.5 py-0.5 rounded text-xs font-mono text-text-primary">
                    r_ij = min(x_j) / x_ij
                  </code>
                </li>
        ...
        {activeTab === 'excluded' && (
          <table className="min-w-full divide-y divide-border-default text-sm">
            ...
            <tbody className="divide-y divide-border-default bg-bg-surface">
              {excluded.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-6 py-8 text-center text-success font-medium">
                    Semua produk memenuhi syarat kalkulasi ✓
                  </td>
                </tr>
              ) : (
